...
class VoiceOrchestrator(TaskRouter):
    ...
    async def route_voice_task(
        self, session_id: str, intent: str, audio_data: bytes, context: Optional[Dict] = None
        ...
        classified_intent = (
            self._classify_voice_intent(stt_result.text, context)
            if not intent or intent == "interruption"
            else intent
        )
    ...
    async def handle_barge_in(
        ...
        async for chunk in self.route_voice_task(
            session_id, "interruption", interruption_audio, context
        ):
            yield chunk
