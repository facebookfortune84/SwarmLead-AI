...
class MonetaryRulesEngine:
    ...
    def authorize_spend(
        ...
        with self._lock:
            ...
            if counterparty.lower() not in {
                c.lower() for c in self.config.allowlisted_counterparties
            }:
                return False
        ...
        with self._lock:
            ...
            for agent_id in list(self._session_spend.keys()):
                ...
                if spent > 0:  # Only check sessions with spend
                    self._reconcile(agent_id, spent, duration)
    ...
    def authorize_spend_with_rail_type(
        self, agent_id: str, amount_usd: float, counterparty: str, rail: RailType
    ) -> bool:
