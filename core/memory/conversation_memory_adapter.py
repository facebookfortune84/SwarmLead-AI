...
class ConversationMemoryAdapter:
    ...
    def get_session_stats(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_turns": sum(1 for t in self._session_cache.get(f"session:{session_id}", []) if t.get("role") == "user"),
            "assistant_turns": sum(1 for t in self._session_cache.get(f"session:{session_id}", []) if t.get("role") == "assistant"),
            "total_chars": sum(len(t.get("text", "")) for t in self._session_cache.get(f"session:{session_id}", [])),
            "last_activity": turns[-1].get("timestamp") if self._session_cache.get(f"session:{session_id}") else None
        }
