...
class OnboardingAgent(StrategyAgent):
    ...
    async def process_step(self, step: str, input_data: Dict) -> Dict:
        ...
        if missing:
            return {"error": f"Missing required fields: {missing}", "step": step}
        ...
        if next_step and self.voice_agent:
            ...
            await self.voice_agent.text_to_speech(next_config.voice_prompt)
    ...
    async def get_progress(self, session_id: str) -> Dict:
        ...
        return {
            ...
            "progress_percent": len(self.session_data.get("completed_steps", []))
            / len(self.STEPS)
            * 100,
        }
