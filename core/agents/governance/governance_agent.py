...
class ComplianceResult:
    ...
    def __init__(
        self, compliant: bool, violation: str = "", article: str = "", details: Dict = None
    ):
            ...
            "sections": {
                ...
                "4": {  # Human Oversight
                    ...
                    "4.4": {
                        "friction_model": {
                            "fast": ["routine"],
                            "genuine": ["legal", "financial", "launch"],
                        }
                    },
    ...
    def check_action(self, action: "AgentAction") -> "ComplianceResult":
        ...
        if not action.agent_id or not action.trace_id:
            return ComplianceResult(
                False,
                "Missing agent_id or trace_id",
                "§3",
                {"agent_id": action.agent_id, "trace_id": action.trace_id},
            )
        ...
        if action.accesses_data and not action.tenant_scoped:
            return ComplianceResult(
                False,
                "Cross-tenant data access without tenant scoping",
                "§4.6",
                {"accesses_data": action.accesses_data, "tenant_scoped": action.tenant_scoped},
            )
        ...
        if action.domain:
            ...
            if not allowed:
                return ComplianceResult(
                    False,
                    f"Domain {action.domain} not allowed for agent",
                    "§5",
                    {"agent_id": action.agent_id, "domain": action.domain},
                )
        ...
        if action.spend_usd:
            if not self._check_monetary_rules(action):
                return ComplianceResult(
                    False,
                    "Monetary rule violation",
                    "§12",
                    {"spend_usd": action.spend_usd, "counterparty": action.counterparty},
                )

        # §13: Agent identity
        if not action.agent_identity_valid:
            return ComplianceResult(
                False, "Invalid or missing agent identity", "§13", {"agent_id": action.agent_id}
            )
...
class TriggerEvaluator:
    ...
    def evaluate(self, action: "AgentAction") -> List[str]:
        ...
        if action.spend_usd:
            ...
            if (
                action.cumulative_spend_usd
                and action.cumulative_spend_usd >= self.TRIGGERS["dollar_value"]["threshold_usd"]
            ):
    ...
    def pre_check(self, action: AgentAction) -> "ComplianceResult":
        ...
        if triggers:
            if not action.human_review_completed:
                return ComplianceResult(
                    False,
                    f"Triggers require genuine review: {triggers}",
                    "§5.1",
                    {"triggers": triggers},
                )
