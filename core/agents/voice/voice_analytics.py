...
from core.monitoring.metrics_collector import (
    record_voice_session, voice_sessions_total, voice_barge_in_latency
)
...
class VoiceAnalytics:
    ...
    def record_turn(self, session_id: str, latency_ms: float = 0, stt_ms: float = 0, tts_ms: float = 0):
    ...
    def get_aggregate_stats(self) -> Dict[str, Any]:
        ...
        if completed > 0:
            avg_duration = sum(
                s.duration_seconds for s in self._sessions.values() if s.ended_at
            ) / completed
        ...
        return {
            ...
            "avg_turns_per_session": sum(s.turn_count for s in self._sessions.values()) / len(self._sessions) if self._sessions else 0
        }
