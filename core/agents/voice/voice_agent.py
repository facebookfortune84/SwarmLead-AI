...
class VoiceAgent(OutreachAgent):
    ...
    async def process_voice_input(
        self, audio_stream: bytes, session_id: str, context: Optional[Dict[str, Any]] = None
        ...
        await self.memory_adapter.store_turn(
            session_id=session_id, role="assistant", text=response_text, audio_meta={}
        )
    ...
    async def _process_with_voice_context(self, text: str, context: Dict[str, Any]) -> str:
    ...
    async def handle_interruption(
        self, session_id: str, interruption_audio: bytes
    ) -> AsyncGenerator[bytes, None]:
        """
        ...
        response_text = await self._process_with_voice_context(
            user_text, {"interruption": True, "session_id": session_id}
        )
